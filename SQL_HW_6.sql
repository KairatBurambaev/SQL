...
-- – обязательно).Создать процедуру, которая решает следующую задачу
...
REPLACE PROCEDURE move_user_to_old_table(user_id INTEGER) AS
BEGIN
   BEGIN
      SELECT * INTO STRICT old_user FROM users WHERE id = user_id;

      INSERT INTO users_old (id, firstname, lastname, email)
         VALUES (old_user.id, old_user.firstname, old_user.lastname, old_user.email);
...
BEGIN
  ...
  ELSIF current_hour >= 12 AND current_hour < 18 THEN
    RETURN 'Добрый день';
  ELSIF current_hour >= 18 OR current_hour < 6 THEN
    RETURN 'Добрый вечер';
  END IF;
